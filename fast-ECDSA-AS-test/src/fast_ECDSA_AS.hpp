...
void fast_ECDSA_AS_PP_new(fast_ECDSA_AS_PP &pp)
{ 
...
/* free memory of PP */ 
void fast_ECDSA_AS_PP_free(fast_ECDSA_AS_PP &pp)
{ 
    EC_POINT_free(pp.g);
}


void fast_ECDSA_AS_KP_new(fast_ECDSA_AS_KP &keypair)
{
...
void fast_ECDSA_AS_KP_free(fast_ECDSA_AS_KP &keypair)
{
    EC_POINT_free(keypair.pk); 
    BN_free(keypair.sk);
    BN_free(keypair.pk_x);
    BN_free(keypair.pk_y);
}
...
void fast_ECDSA_AS_SIG_new(fast_ECDSA_AS_SIG &SIG)
{
    SIG.r = BN_new(); 
    SIG.z = BN_new();
...
void fast_ECDSA_AS_SIG_free(fast_ECDSA_AS_SIG &SIG)
{
    BN_free(SIG.r); 
    BN_free(SIG.z);
    EC_POINT_free(SIG.Z);
...
void fast_ECDSA_AS_SIG_serialize(fast_ECDSA_AS_SIG &SIG, ofstream &fout)
{
    BN_serialize(SIG.r, fout); 
    BN_serialize(SIG.z, fout); 
} 

void fast_ECDSA_AS_SIG_deserialize(fast_ECDSA_AS_SIG &SIG, ifstream &fin)
{
...
/* Setup algorithm */ 
void fast_ECDSA_AS_Setup(fast_ECDSA_AS_PP &pp)
{ 
    EC_POINT_copy(pp.g, generator); 

//    #ifdef DEBUG
//    cout << "generate the public parameters for fast_ECDSA_AS Signature >>>" << endl; 
//    fast_ECDSA_AS_PP_print(pp); 
//    #endif
}

/* KeyGen algorithm */ 
void fast_ECDSA_AS_KeyGen(fast_ECDSA_AS_PP &pp, fast_ECDSA_AS_KP &keypair)
{ 
    BN_random(keypair.sk); // sk \sample Z_p
    EC_POINT_mul(group, keypair.pk, keypair.sk, NULL, NULL, bn_ctx); // pk = g^sk  
    EC_POINT_get_affine_coordinates_GFp(group, keypair.pk, keypair.pk_x, keypair.pk_y, bn_ctx);
//    #ifdef DEBUG
//    cout << "key generation finished >>>" << endl;  
//    fast_ECDSA_AS_KP_print(keypair); 
//    #endif
}

void IY_Gen(fast_ECDSA_AS_PP &pp, IY &statwit)
{
...
void fast_ECDSA_AS_Sign(fast_ECDSA_AS_PP &pp, fast_ECDSA_AS_KP &keypair, EC_POINT *&Y, string &message, fast_ECDSA_AS_SIG &SIG)
{
    fast_ECDSA_AS_SIG sig; // define the signature
    ...
    BN_mul(temp_sk, keypair.sk, SIG.r, bn_ctx); //temp_sk=sk*r_x
    ...
    EC_POINT_mul(group, SIG.Z, NULL, Y, keypair.sk, bn_ctx); // SIG.Z=skY=xY=xyG
    ...
    p_r_x1=BN_bn2hex(keypair.pk_x);
    p_r_y1=BN_bn2hex(keypair.pk_y);
...
//    cout << "s_r_yy2 >>>"<< s_r_yy2 << endl;
    ...
    BN_mod_mul(temp_a, keypair.sk, SIG.a, order, bn_ctx); // temp_a = sk*a=a*x; 

    BN_mod_add(SIG.b,kk,temp_a,order,bn_ctx);//SIG.b=kk+temp_a=kk+x*a

//    #ifdef DEBUG
//        cout << "fast_ECDSA_AS signature generation finishes >>>" << endl;
//        fast_ECDSA_AS_SIG_print(SIG);  
//    #endif
...
bool fast_ECDSA_AS_Verify(fast_ECDSA_AS_PP &pp, BIGNUM *&pk_x,BIGNUM *&pk_y, EC_POINT *&pk, EC_POINT *&Y, string &message, fast_ECDSA_AS_SIG &SIG)
{
    bool Validity;       
    ...
    p_r_a1=BN_bn2hex(pk_x);
    p_r_b1=BN_bn2hex(pk_y);
