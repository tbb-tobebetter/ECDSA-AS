...
#include "../src/fast_ECDSA_AS.hpp"

void test_fast_ECDSA_AS()
{
    cout << "begin the basic correctness test >>>" << endl; 
    
    fast_ECDSA_AS_PP pp; 
    fast_ECDSA_AS_PP_new(pp); 
    fast_ECDSA_AS_Setup(pp); 
    ...
    fast_ECDSA_AS_KP keypair[TEST_NUM];
    for(auto i = 0; i < TEST_NUM; i++)
    {
    fast_ECDSA_AS_KP_new(keypair[i]);
    }
    ...
    for(auto i = 0; i < TEST_NUM; i++)
    {
    fast_ECDSA_AS_KeyGen(pp, keypair[i]); 
    }
    ...
    fast_ECDSA_AS_SIG SIG[TEST_NUM];
    for(auto i = 0; i < TEST_NUM; i++)
    { 
    fast_ECDSA_AS_SIG_new(SIG[i]); 
    }
    ...
    for(auto i = 0; i < TEST_NUM; i++)
    { 
    fast_ECDSA_AS_Sign(pp, keypair[i], statwit[i].Y, message, SIG[i]);
    }
    end_time = chrono::steady_clock::now(); 
    running_time = end_time - start_time;
    ...
    for(auto i = 0; i < TEST_NUM; i++)
    { 
    fast_ECDSA_AS_Verify(pp, keypair[i].pk_x,keypair[i].pk_y,keypair[i].pk, statwit[i].Y, message, SIG[i]);
    }
    end_time = chrono::steady_clock::now(); 
    running_time = end_time - start_time;
...
{  
    ...
    SplitLine_print('-'); 
    cout << "fast_ECDSA_AS Signature test begins >>>>>>" << endl; 
    SplitLine_print('-'); 

    test_fast_ECDSA_AS();

    SplitLine_print('-'); 
    cout << "fast_ECDSA_AS Signature test finishes <<<<<<" << endl; 
    SplitLine_print('-'); 
